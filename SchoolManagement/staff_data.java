package SchoolManagement;

import java.awt.*;
import javax.swing.*;
import java.awt.event.*;
import java.sql.*;

class staff_data implements ActionListener{
    JFrame f;
    JLabel id8,id,aid,id1,aid1,id2,aid2,id3,aid3,id4,aid4,id5,aid5,id6,aid6,id7,aid7,id9,id10,id11,id12,id13,id14,id15,id16,id17,lab,aid8, aid9, aid10;
    String staff_id,name,age,address,dob,phone,salary,email,gender,qual,dep,sub;
    JButton b1,b2;
    ImageIcon icon;
    int flag = 0;

    staff_data(String e_id){
        try{
            conn con = new conn();
            String str = "select * from staff where id = '"+e_id+"'";
            ResultSet rs= con.s.executeQuery(str);
            int i = 0;
            while(rs.next()){

                staff_id = rs.getString(1);
                name = rs.getString(2);
                age = rs.getString(3);
                address = rs.getString(4);
                dob = rs.getString(5);
                phone = rs.getString(6);
                salary = rs.getString(7);
                email = rs.getString(8);
                gender = rs.getString(9);
                qual= rs.getString(10);
                dep= rs.getString(11);
                //sub = rs.getString(12);
                flag = 0;
                i=1;
            }
                
            str ="select subj from sub where sid = '"+e_id+"'";
            rs = con.s.executeQuery(str);
            if(rs.next())
            {
                flag =1;
                sub = rs.getString(1);
                id12.setVisible(true);
                aid10.setVisible(true);
            } 
            if(i==0) 
               JOptionPane.showMessageDialog(null,"id not found!!");
            
        }catch(Exception e){
            e.printStackTrace();
        }
 
        f=new JFrame("Staff Data");
        f.setVisible(true);
        f.setSize(595,800);
        f.setLocation(450,200);
        f.setBackground(Color.white);
        f.setLayout(null);


        id9=new JLabel();
        id9.setBounds(0,0,595,642);
        id9.setLayout(null);
        id9.setBackground(Color.WHITE);
        ImageIcon img=new ImageIcon("a.jpg");
        id9.setIcon(img);

        id8 = new JLabel("Staff Details");
        id8.setBounds(50,10,250,30);
        f.add(id8);
        id8.setFont(new Font("serif",Font.BOLD,25));
        id9.add(id8);
        f.add(id9);

        id = new JLabel("Staff Id:");
        id.setBounds(50,70,120,30);
        id.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id);

        aid = new JLabel(staff_id);
        aid.setBounds(200,70,200,30);
        aid.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid);

        id1 = new JLabel("Name:");
        id1.setBounds(50,120,100,30);
        id1.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id1);

        aid1 = new JLabel(name);
        aid1.setBounds(200,120,300,30);
        aid1.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid1);

  
        id2 = new JLabel("age: "); 
        id2.setBounds(50,170,200,30);
        id2.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id2);

        aid2 = new JLabel(age);
        aid2.setBounds(200,170,300,30);
        aid2.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid2);

        id3= new JLabel("Address:");
        id3.setBounds(50,220,100,30);
        id3.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id3);

        aid3= new JLabel(address);
        aid3.setBounds(200,220,300,30);
        aid3.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid3);


        id4= new JLabel("Mobile No:");  
        id4.setBounds(50,270,100,30);
        id4.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id4);

        aid4= new JLabel(phone);
        aid4.setBounds(200,270,300,30); 
        aid4.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid4);

        
        id5= new JLabel("salary: ");
        id5.setBounds(50,320,100,30);
        id5.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id5);

        aid5= new JLabel(salary);
        aid5.setBounds(200,320,300,30);
        aid5.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid5);

        
        id6= new JLabel("Email: ");
        id6.setBounds(50,370,100,30);
        id6.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id6);

        aid6= new JLabel(email);
        aid6.setBounds(200,370,300,30); 
        aid6.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid6);


        id7= new JLabel("Gender:");
        id7.setBounds(50,420,100,30);
        id7.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id7);

        aid7= new JLabel(gender);
        aid7.setBounds(200,420,300,30);
        aid7.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid7);

        id10= new JLabel("qualification:");
        id10.setBounds(50,470,100,30);
        id10.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id10);

        aid8= new JLabel(qual);
        aid8.setBounds(200,470,300,30);
        aid8.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid8);

        id11= new JLabel("department:");
        id11.setBounds(50,520,100,30);
        id11.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id11);

        aid9= new JLabel(dep);
        aid9.setBounds(200,520,300,30);
        aid9.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid9);

        id12= new JLabel("Subject:");
        id12.setBounds(50,570,100,30);
        id12.setFont(new Font("serif",Font.BOLD,20));
        id9.add(id12);
        if(flag==0)
            id12.setVisible(false);

        aid10= new JLabel(sub);
        aid10.setBounds(200,570,300,30);
        aid10.setFont(new Font("serif",Font.BOLD,20));
        id9.add(aid10);
        if(flag==0)
            aid10.setVisible(false);

        b1=new JButton("Print");
        b1.setBackground(Color.BLACK);
        b1.setForeground(Color.WHITE);
        b1.setBounds(100,670,100,30);
        b1.addActionListener(this);
        id9.add(b1);

        b2=new JButton("cancel");
        b2.setBackground(Color.BLACK);
        b2.setForeground(Color.WHITE);
        b2.setBounds(250,670,100,30);
        b2.addActionListener(this);
        id9.add(b2);


    }

    public void actionPerformed(ActionEvent ae){

        if(ae.getSource()==b1){
            JOptionPane.showMessageDialog(null,"printed successfully");
            f.setVisible(false);
        }
        if(ae.getSource()==b2){
            f.setVisible(false);
        }
    }
    public static void main(String[] args){
        new staff_data("1");
    }

}