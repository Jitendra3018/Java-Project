...
class removestaff implements ActionListener{
    JFrame f;
    JTextField t;
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b){
            try{
                ...
                conn con = new conn();
                String str = "select name,department,email from staff where id ='"+t.getText()+"' ";
        ...
        if(ae.getSource()==b1){
            try{
                conn con = new conn();
                String str = "select department from staff where id ="+t.getText();
                ResultSet rs = con.s.executeQuery(str);
                ...
                if(dep.equals("administration"))
                    login="loginadmin ";
                else if(dep.equals("faculty"))
                {
                    str = "delete from sub where sid = "+t.getText();
                    con.s.executeUpdate(str);
                    login="loginfaculty ";
                }   
                if(dep.equals("administration")||dep.equals("faculty"))
                {
                    str = "delete from " + login + "where userid = " + t.getText();
                    con.s.executeUpdate(str);
                  }
                }
                str = "delete from staff where id = "+t.getText();
                con.s.executeUpdate(str);                
