...
class removestudent implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b){
            try{
                conn con = new conn();
                String str = "select name,class,email from student where id ="+t.getText();
        ...
        if(ae.getSource()==b1){
            try{
                conn con = new conn();
                String str = "delete from student where id ="+t.getText();
                con.s.executeUpdate(str);
                str = "delete from loginstudent where userid ="+t.getText();
                con.s.executeUpdate(str);
                str = "delete from marks where userid ="+t.getText();
                con.s.executeUpdate(str);
